...
*/
	 ...
	 */
	public function indexFiles (array $fileIds, \OC_EventSource $eventSource = null, $metadata = null) {
		...
		foreach ($fileIds as $id) {
			...
			try {
				...
				if ($this->indexFile($node, false, $metadata)) {
	 ...
	 */
	public function indexFile(File $file, $commit = true, $metadata = null) {
			...
			if ($metadata != null) {
				foreach ($metadata as $value) {
					$doc->addField(Document\Field::Keyword($value["name"], $value["value"]));
				}
			}
	...
	public function removeFileIndex($fileId) {
		try {
			//get filestatus from db/statusmapper
			$status = $this->mapper->getFromFileId($fileId);
			//delete status from the table
			if ($status !== false)
